...
def search_privacy_law(query: str) -> str:
    """Search privacy and data protection knowledge base for relevant rules.

    Args:
        query: Natural language query about privacy, GDPR, consent, or data breach.
    """
    knowledge = [
        (
            ["gdpr", "privacy", "data", "consent", "personal"],
            "GDPR requires a lawful basis for processing personal data, transparency notices, "
            "data minimisation, purpose limitation, and respect for data subject rights. "
            "Serious violations may lead to fines up to EUR 20M or 4% of global annual turnover.",
        ),
        (
            ["breach", "leak", "security", "incident", "notification"],
            "Data breach response usually requires containment, impact assessment, regulator "
            "notification, and user notification when the breach risks individual rights. "
            "GDPR supervisory authority notification is generally required within 72 hours when applicable.",
        ),
        (
            ["ccpa", "cpra", "california", "consumer"],
            "CCPA/CPRA gives consumers rights to know, delete, correct, opt out of sale/sharing, "
            "and limit use of sensitive personal information. Intentional violations can trigger civil penalties.",
        ),
    ]
    query_lower = query.lower()
    results = []
...
async def check_routing(state: LegalState) -> dict:
    ...
    messages = [
        SystemMessage(
            content=(
                ...
                'needs_privacy = true → question involves data privacy, GDPR, consent, personal data, data breach'
            )
        ),
        HumanMessage(content=state["question"]),
    ]
    ...
    privacy_keywords = ["data", "privacy", "gdpr", "personal", "consent", "breach"]
    keyword_needs_privacy = any(kw in state["question"].lower() for kw in privacy_keywords)
    needs_privacy = bool(parsed.get("needs_privacy", keyword_needs_privacy)) or keyword_needs_privacy
    print(
        "  [Node: check_routing] "
...
async def call_privacy_specialist(state: LegalState) -> dict:
    ...
    privacy_prompt = (
        "You are a privacy and data protection attorney specialising in GDPR, CCPA/CPRA, "
        "personal data processing, consent, breach notification, and data subject rights. "
...
QUESTION = (
    "If a technology company shares customer personal data without consent, suffers a data breach, "
    "and avoids taxes on overseas revenue, what are the legal, tax, compliance, and privacy consequences?"
)
